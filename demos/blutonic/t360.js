...
var T360 = (function(){
  ...
  me.init = function(config) {
    ...
    adapt = config.adapt || 'integration_test';
    customPixel = config.customPixel || '';
    skin = config.skin || 'http://d2s1vwfhtsw5uw.cloudfront.net/assets/skin/tc_skin.xml';
    logoFile = config.logoFile || 'http://d2s1vwfhtsw5uw.cloudfront.net/assets/T360logo_50.png';
    ...
    muteButtonId = config.muteButtonId || 't360_muteButton';
    muteActiveClass = config.muteActiveClass || 't360_muteActive';
    muteHideClass = config.muteHideClass || 't360_muteHide';
  ...
  function loadStyles() {
    ...
    styles.innerHTML = '.'+playlistContainer+' {position: absolute;top:0;right: 5px;display: none;} .'+thumbnailContainer+' {zoom: 1;filter: alpha(opacity=50);opacity: 0.5;} .'+thumbnailContainer+':hover {zoom: 1;filter: alpha(opacity=100);opacity: 1;} .'+playlistContainer+' div:hover, .'+playlistContainer+' span:hover {color: #ccc !important;cursor: pointer;} .'+muteActiveClass+' {display:block;} .'+muteHideClass+' {display:none;}';
  ...
  function instantiateDesktop() {
    ...
    player = jwplayer(videoArea).setup({
      ...
      advertising: {
        ...
        //tag: 'http://t-ads.adap.tv/a/t/tribal360llc?artEid=vast_2_0_vpaid_failover&categories=1',
        tag: 'http://t-ads.adap.tv/a/t/tribal360llc?artEid=vast_2_0&categories=1',
  ...
  function openxPixelSuccess(respObj) {
    ...
    var xml = xmlFromString(respObj),
      impression = xml.getElementsByTagName("Impression"),
      img;
  ...
  function xmlFromString(string) {
    ...
    if (window.DOMParser) { // all browsers, except IE before version 9
      ...
      if (xmlDoc.parseError && xmlDoc.parseError.errorCode !== 0) {
        //console.log("XML Parsing Error: " + xmlDoc.parseError.reason
  ...
  function playlistMouseover() {
    if(!isAdPlaying){
      ...
      document.getElementById(muteButtonId).setAttribute('class', muteActiveClass);
    }
  }

  function playlistMouseout() {
    document.getElementsByClassName(playlistContainer)[0].setAttribute('style', 'display:none;');
    document.getElementById(muteButtonId).setAttribute('class', muteHideClass);
  }
  ...
  function createMuteButton() {
    ...
    button.setAttribute('id', muteButtonId);
    button.setAttribute('class', muteHideClass);
  ...
  function hideMuteButton() {
    ...
    var muteButton = document.getElementById(muteButtonId);
    muteButton.setAttribute('class', muteHideClass);
  }
  ...
  function onMute() {
    ...
    if(!player.getMute()) {
      player.setMute(true);
      document.getElementById(muteButtonId).innerHTML = "UNMUTE";
    } else {
      player.setMute(false);
      document.getElementById(muteButtonId).innerHTML = "MUTE";
    }
  }
  ...
  function adHasEnded() {
    ...
    // hide mute button since controls are back
    hideMuteButton();
